...
Description|Adds a register and an unsigned immediate value and stores the result in 
a register  
Operation|$t = $s + imm; advance_pc (4);  
...
Description|Bitwise ands a register and an immediate value and stores the result in a 
register  
Operation|$t = $s & imm; advance_pc (4);  
Syntax|andi $t, $s, imm  
...
Operation|
if $s >= 0 $31 = PC + 8 (or nPC + 4); advance_pc (offset << 2)); else 
advance_pc (4);  
...
BLTZAL -- Branch on less than zero and link Description|
...
Operation|
$31 = PC + 8 (or nPC + 4); PC = nPC; nPC = (PC & 0xf0000000) | 
(target << 2);  
Syntax|jal target  Encoding|0000 11ii iiii iiii iiii iiii iiii iiii  
...
Operation|$t = $s | imm; advance_pc (4);  Syntax|ori $t, $s, imm  
